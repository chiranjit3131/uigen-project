...
function formatToolLabel(tool: {
  ...
  const basename = (p?: string) =>
    p ? p.split("/").filter(Boolean).pop() ?? "file" : "file";

  const verbs: Record<string, { now: string; done: string }> = {
    create: { now: "Creating", done: "Created" },
    str_replace: { now: "Editing", done: "Edited" },
    insert: { now: "Editing", done: "Edited" },
    view: { now: "Viewing", done: "Viewed" },
    undo_edit: { now: "Reverting", done: "Reverted" },
    rename: { now: "Renaming", done: "Renamed" },
    delete: { now: "Deleting", done: "Deleted" },
  };

  if (!command || !(command in verbs)) {
    return "Working…";
  }

  const verb = isDone ? verbs[command].done : verbs[command].now;

  if (command === "rename" && isDone && newPath) {
    return `${verb} ${basename(path)} → ${basename(newPath)}`;
  }
